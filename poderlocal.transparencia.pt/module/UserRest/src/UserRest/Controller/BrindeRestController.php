...
class BrindeRestController extends AbstractRestfulController
{ 
    ...
    public function getList()
    {
    	...
    	$brindes = $em->getRepository('Application\Entity\PostBrinde')->findAll();
    	...
    	foreach ($brindes as $brinde){
    		...
    		$results[$i++]=array(
    			'idBrinde'=> $brinde->getIdBrinde(),
    			'nome' => $brinde->getNome(),
    			'desc'=> $brinde->getDescricao(),
    ...
    public function get($id)
    {
    	...
    	$em = $this->getServiceLocator()
    	->get('doctrine.entitymanager.orm_default');
    	$brinde = $em->getRepository('Application\Entity\Brinde')->find($id);
    ...
    public function create($datas)
    {
		...
		if($lista==null)
			return new JsonModel(array('SUCCESS'=>'FAIL','MESSAGE'=>'idBrinde Nao encontrado'));
		...
		$sucess = file_put_contents('public/assets/brinde/'.$date.'-brinde.jpg', $data);
    	if($sucess){
    		...
    		$image = $imagine->open($folder.$date.'-brinde.jpg');
    		$image->thumbnail($size, $mode)->save($folder .'/thumbs/'.$date.'-brinde.jpg');
    		
    		$brinde = new PostBrinde();
    		$brinde->setIdUser($u);
    		$brinde->setIdLista($l); 
    		$brinde->setNome($_POST['nome']);
    		if(isset($_POST['desc']))   		
    		$brinde->setDescricao($_POST['desc']);
    		$brinde->setData($date);
    		$brinde->setImage($date.'-brinde.jpg');    		
    		$brinde->setActive(0);
