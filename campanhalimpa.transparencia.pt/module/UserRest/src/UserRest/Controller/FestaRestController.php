...
class FestaRestController extends AbstractRestfulController
{ 
    ...
    public function getList()
    {
    	...
    	$festas = $em->getRepository('Application\Entity\Festa')->findAll();
    	...
    	foreach ($festas as $festa){
    		...
    		$results[$i++]=array(
    			'idFesta'=> $festa->getIdFesta(),
    			'nome' => $festa->getNome(),
    			'morada'=> $festa->getMorada(),
    ...
    public function get($id)
    {
    	...
    	$em = $this->getServiceLocator()
    	->get('doctrine.entitymanager.orm_default');
    	$festa = $em->getRepository('Application\Entity\Festa')->find($id);
...
public function create($datas)
    {
		...
		$concelho = (int)$_POST['concelho']-1;
		$partido = $_POST['partido'];
		$distrito = $_POST['distrito'];
		$morada = $this->getAdress($corX, $corY);
		...
		$sucess = file_put_contents('public/assets/festa/'.$date.'-festa.jpg', $data);
    	if($sucess){
    		...
    		$image = $imagine->open($folder.$date.'-festa.jpg');
    		$image->thumbnail($size, $mode)->save($folder .'/thumbs/'.$date.'-festa.jpg');
    		
    		$exFesta = $em->getRepository('Application\Entity\Festa')->findBy(array('idLista'=>$l->getIdLista()));
    	...
    	/////////////////////////////////////////////////////////////////////////////////////	
    		...
    		$festa->setIdUser($u);
    		$festa->setIdLista($l);
    		$festa->setNome($_POST['nome']);
    		$festa->setData($date);
    		$festa->setCorX($corX);
    		$festa->setCorY($corY);
    		if (isset($_POST['desc']))
    		$festa->setDescricao($_POST['desc']);
    		if($morada==NULL){
    			$festa->setMorada('sem morada');
    			$festa->setGmaps('sem morada');
    		}else{
    			$festa->setMorada($morada);
    			$festa->setGmaps($morada);
    		}
    		...
    		$image = new FestaImg();
    		$image->setIdFesta($festa);
    		$image->setImagem($date.'-festa.jpg');
    		$image->setActive(0);
    		$em->persist($image);
    		$em->flush();
