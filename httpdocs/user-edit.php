...
$donativos = $wpdb->get_row("SELECT SUM(total_amount) FROM civicrm_contribution WHERE contact_id = '{$id_contatos->id}' AND contribution_page_id ='4' || contribution_page_id ='8'",ARRAY_A);
//echo $ano_atual;
/*	 
...
<?php
	 	echo "<div  class='row'style='color: #3a87ad; width: 80%; background-color: #d9edf7; border-color: #bce8f1; border-radius: 10px; padding: 8px 35px 8px 14px;'>";
	    	 if(empty($donativos['SUM(total_amount)'])){
	    	 	...
	    	 	echo "Total de donativos efetuados até a data: ".$donativos['SUM(total_amount)']."€";
	    	}
