...
echo 'alert("Caros Associados, de momento estamos a proceder a uma actualização das vossas fichas individuais, pelo que não será possível o pagamento das quotas em atraso. Brevemente, receberão nos vossos emails uma mensagem da TIAC solicitando a regularização das quotas. Pedimos desculpa pelo incómodo e agradecemos a vossa compreensão.")';
echo '</script>';*/
...
$ano_atual= date("Y");
$quotas_2013 = $wpdb->get_row("SELECT receive_date,contribution_status_id FROM civicrm_contribution WHERE contact_id = {$id_contatos->id} AND contribution_page_id='9' AND receive_date LIKE '%2013%' ORDER BY receive_date DESC",ARRAY_A);
$quotas_2014 = $wpdb->get_row("SELECT receive_date,contribution_status_id FROM civicrm_contribution WHERE contact_id = {$id_contatos->id} AND contribution_page_id='9' AND receive_date LIKE '%2014%' ORDER BY receive_date DESC",ARRAY_A);
$quotas_2015 = $wpdb->get_row("SELECT receive_date,contribution_status_id FROM civicrm_contribution WHERE contact_id = {$id_contatos->id} AND contribution_page_id='9' AND receive_date LIKE '%2015%' ORDER BY receive_date DESC",ARRAY_A);
$quotas_atuais = $wpdb->get_row("SELECT receive_date,contribution_status_id FROM civicrm_contribution WHERE contact_id = {$id_contatos->id} AND contribution_page_id='9' AND receive_date LIKE '%{$ano_atual}%' ORDER BY receive_date DESC",ARRAY_A);
...
<form action="#" method="post">
	<button class='btn btn-default col-xs-6' style='cursor:pointer;width:200px;height:50px;background: #1e5799; 
...
<?php
    	echo "<div style='color: #3a87ad; width: 32%;  padding: 8px 35px 8px 14px;'>";
			...
			if($contribution_status_id_2013 == 1 && $datas_pagamento_2013 == "2013"){
				echo "<div style='color:green;'>Quota regularizada de " . $datas_pagamento_2013 . "</div>";
			}

			if($contribution_status_id_2014 == 1 && $datas_pagamento_2014 == "2014"){
				echo "<div style='color:green;'>Quota regularizada de " . $datas_pagamento_2014 . "</div>";
			}

			if($contribution_status_id_2015 == 1 && $datas_pagamento_2015 == "2015"){
				echo "<div style='color:green;'>Quota regularizada de " . $datas_pagamento_2015 . "</div>";
			}

			if($contribution_status_id_quotas_atuais == 1 && $datas_pagamento_quotas_atuais == $datas_pagamento_quotas_atuais){
				echo "<div style='color:green;'>Quota regularizada de " . $datas_pagamento_quotas_atuais . "</div>";
			}
  
			if($contribution_status_id_2013 == 2 && $datas_pagamento_2013 == "2013"){
				echo "<div style='color:red;'>Quota não regularizada de " . $datas_pagamento_2013 . "</div>";
			}

			if($contribution_status_id_2014 != 1 && $datas_pagamento_2014 == "2014"){
			echo "<div style='color:red;'>Quota não regularizada de " . $datas_pagamento_2014 . "</div>";
			}

			if($contribution_status_id_2015 != 1 && $datas_pagamento_2015 == "2015"){
			echo "<div style='color:red;'>Quota não regularizada de " . $datas_pagamento_2015 . "</div>";
			}

			if($contribution_status_id_quotas_atuais != 1 && $datas_pagamento_quotas_atuais == $datas_pagamento_quotas_atuais){
			$wpdb->query("INSERT INTO wp_users (external_identifier) VALUES('$numero_associado') ");
			 echo "<div style='color:red;'>Quota não regularizada de " .$ano_atual . "</div>";
			}
    		echo "</div>";
  ...
  $quota_paga_mais_recente =  $wpdb->get_row("SELECT MAX(receive_date) FROM civicrm_contribution WHERE contact_id = {$id_contatos->id} AND contribution_page_id='9' AND contribution_status_id = '1'",ARRAY_A);
...
<form action="#" method="post">
	<button class='btn btn-default col-xs-6' style='cursor:pointer;width:200px;height:50px;margin-top:0px;background: #f1da36;
...
//print_r ($googleWallet);
	 	echo "<div  class='row'style='color: green; width: 32%;  padding: 8px 35px 8px 14px;'>";
	    	 if(empty($donativos['SUM(total_amount)']) && empty($googleWallet['SUM(total_amount)'])){
	    	 	echo "Total de donativos : 0.00€";
	    	 }else{
	    	 	echo "Total de donativos: ".$total."€";
	    	}
...
<?php if ( ! ( IS_PROFILE_PAGE && ! $user_can_edit ) ) :  $data = date('Y-m-d H:i:s');
	
//$wpdb->query("INSERT INTO civicrm_contribution (contact_id) VALUES('5000') ");
	//echo "IF";
...
background: linear-gradient(to bottom,  #f1da36 21%,#fcf7cc 69%);
filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#f1da36', endColorstr='#fcf7cc',GradientType=0 );
' type="submit" name="botaoTeste">
	<h2 style='color:#1e5799;'>Testes</h2>
	</button>

        <?php if(isset($_POST['botaoTeste'])){

wp_redirect($url."/testes");
...
<?php      $nif = $wpdb->get_row("SELECT nif_3 FROM civicrm_value_nif_3 WHERE external_identifier = {$external_id->external_identifier}");
		   ...
		   //$socios_proponentes = $wpdb->get_row("SELECT outro_18 FROM civicrm_value_outro_15 WHERE entity_id = {$user_id->entity_id}");
	...
	//"SELECT outro_18 FROM civicrm_value_outro_15 WHERE entity_id = {$entity_id->entity_id}"
	?>
