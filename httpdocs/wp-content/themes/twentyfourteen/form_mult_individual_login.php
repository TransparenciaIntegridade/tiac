...
<?php
 ...
 function binif_isvalid($nif){
   ...
   if(empty($donativo) ){
        $erros[] = "<i style='color:#ff0000'>Por favor insere um valor válido para efetuar o donativo.</i>";

      }
     if($donativo > 5000 ){
        $erros[] = "<i style='color:#ff0000'>Excedeste o valor máximo para um donativo.</i>";

      }
      if($total_final > 5000 ){
        $erros[] = "<i style='color:#ff0000'>Excedeste o limite de donativo para o ano corrente (5000€/Ano).</i>";
         $erros[] = "<i style='color:#ff0000'>Apenas podes fazer um donativo no total de:</i>" .$limite_donativos ."€";
       }
...
$contact_id = $wpdb->get_row("SELECT id FROM civicrm_contact WHERE external_identifier='$external_id->external_identifier'");
 if(empty($erros) == false){
      ...
      $contact_id = $wpdb->get_row("SELECT id FROM civicrm_contact WHERE external_identifier='$external_id->external_identifier'");
      $wpdb->query("INSERT INTO civicrm_contribution (contact_id,financial_type_id,contribution_page_id,payment_instrument_id,receive_date,total_amount,currency,source,contribution_status_id) VALUES ('$contact_id->id','2','4','4','$data_recebimento','$donativo','EUR','Contribuição Online: Doações Indivíduo','2')");
echo "<script>";
...
<p>Insere o valor do donativo</p>
                 <!-- <input type="radio" name="price_9" value="10.00" class="form-text four required">10€<br>
                  <input type="radio" name="price_9" value="20.00" class="form-text four required">20€<br>                 
                   <input type="radio" name="price_9" value="50.00" class="form-text four required">50€<br>-->
                  <input price="[11,&quot;1||&quot;]" size="4" name="price_9" type="text" id="price" class="form-text four required" placeholder="ex: 10, 50, 100...">
                                                        </div>
                <div class="clear"></div>
...
<div id="crm-submit-buttons" class="crm-submit-buttons">
 <div class="g-recaptcha" data-theme="dark" data-sitekey="6LcKdwUTAAAAALbhMtNKePbMnaRFNXDtxVMy4lRz"></div>
  <div style="margin-top:50px"></div>
<span class="crm-button crm-button-type-upload crm-button_qf_Main_upload"><input class="validate form-submit default" name="_qf_Main_upload" value="Confirmar a Contribuição" type="submit" id="_qf_Main_upload-bottom" style="backgroung-color:#CCC"></span>
      </div>
  </div>
